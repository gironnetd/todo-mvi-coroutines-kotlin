...
class AddEditTaskFragment : androidx.fragment.app.Fragment(), MviView<AddEditTaskIntent, AddEditTaskViewState> {
  ...
  private fun bind() {
    ...
    job = viewLifecycleOwner.lifecycleScope.launch {
      viewModel.states().buffer().collect(this@AddEditTaskFragment::render)
    }
  ...
  override fun intents(): Flow<AddEditTaskIntent> {
    return merge(initialIntent(), saveTaskIntent())
  }
   ...
   */
  private fun initialIntent(): Flow<AddEditTaskIntent.InitialIntent> {
    return flowOf(AddEditTaskIntent.InitialIntent(argumentTaskId))
  }

  private fun saveTaskIntent(): Flow<AddEditTaskIntent.SaveTask> {
